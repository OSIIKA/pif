...
get '/fleet' do
  ...
  # 艦艇（UserItem）を取得（A案：UserItem.id を参照）
  @flagship = @user.user_items.find_by(id: @current_fleet.flagship_id)
  @sub_ships = [
    @user.user_items.find_by(id: @current_fleet.sub_ship_1_id),
    @user.user_items.find_by(id: @current_fleet.sub_ship_2_id),
    @user.user_items.find_by(id: @current_fleet.sub_ship_3_id),
    @user.user_items.find_by(id: @current_fleet.sub_ship_4_id),
    @user.user_items.find_by(id: @current_fleet.sub_ship_5_id),
    @user.user_items.find_by(id: @current_fleet.sub_ship_6_id)
  ]

  # ユーザーが所持している艦艇一覧（UserItem.object_id = 0）
  @my_ships = @user.user_items.where(object_id: 0)
  puts "📦 [FLEET] 所持艦数: #{@my_ships.count}"
