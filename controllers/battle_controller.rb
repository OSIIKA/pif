...
post '/battle/start' do
  ...
  (1..6).each do |fleet_num|
    ...
    # 旗艦と随伴艦1〜6をまとめてスキャン
    ships = [
      { ship: fleet_data.flagship, is_flag: true },
      fleet_data.sub_ship_1, fleet_data.sub_ship_2,
      fleet_data.sub_ship_3, fleet_data.sub_ship_4,
      fleet_data.sub_ship_5, fleet_data.sub_ship_6
    ].compact # nil（空スロット）を除外

    ships.each do |item|
      # ループの持ち方によって構造が違う場合、実態に合わせて調整してください
